...
def create_search_session_id() -> str:
...
class MercariScraper:
    ...
    def __init__(self, config: dict):
        ...
        # Используем curl_cffi для обхода TLS fingerprinting на VPS
        self.session = requests.Session(impersonate="chrome120")

        # Настройка прокси из конфига (при наличии)
        ...
        self.session.headers.update(
            {
                ...
                "x-country-code": "JP",  # Рекомендуется использовать JP для jp.mercari.com
                "x-platform": "web",
            }
        )
    ...
    def _build_payload(self, query: dict) -> dict:
        ...
        if query.get("brand_id"):
            ...
            if isinstance(category_id, list):
                search_condition["brandId"] = brand_id
            else:
                search_condition["brandId"] = [brand_id]
    ...
    def _parse_product(self, item: dict) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.error("Failed parsing product", error=str(e))
            return None
    ...
    def search_products(self, query: dict) -> List[Dict]:
        try:
            ...
            payload["laplaceDeviceUuid"] = self.device.get_device_id()

            self.logger.info("Mercari API search", query=query)

            for attempt in range(3):
                try:
                    dpop_token = self.dpop.generate(self.API_URL, "POST")
                except Exception as e:
                    self.logger.error("DPoP generation failed", error=str(e))
                    dpop_token = ""
                ...
                try:
                    ...
                    self.logger.warning(
                        "Mercari request error or timeout",
                        attempt=attempt + 1,
                        error=str(req_err),
                    )
                    time.sleep(3)
                ...
                if response.status_code == 200:
                    ...
                    self.logger.info("Products extracted", count=len(products))
                    return products

                elif response.status_code == 401:
                    self.logger.warning("401 - regenerating DPoP")
                    continue

                elif response.status_code == 429:
                    self.logger.warning("Rate limit")
                    time.sleep(10)
                    continue
            ...
            self.logger.error("Mercari request failed", error=str(e))
            return []
...
if __name__ == "__main__":
    ...
    result = scraper.search_products(
        {"keyword": "shimano alfine", "category_id": 1139}
    )
